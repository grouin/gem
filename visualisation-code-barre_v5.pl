...
foreach my $fichier (@rep) {
    warn "Traite $fichier pour la visualisation\n";
    open(E,'<:utf8',$fichier);
    my $i=0;
    my $tokens="";
    my $note=0;
	...
	chomp $ligne;
	my @cols=split(/\t/,$ligne);
	my $expression=""; if ($cols[7] ne "") { $expression=$cols[2]; }  # 2=forme, 7=lemme
	if ($ligne=~/^(\d)/) { $genre=$1; }
	# Note en fonction de l'émotion identifiée par tour de parole
	if ($cols[5]=~/malveillance/) { $note=-0.1; $tokens.="$expression\, "; $mal="*" if ($afficheMalveillance eq "o"); }
...
print S " <p>Chaque barre verticale correspond &agrave\; un tour de parole exclusivement f&eacute\;minin (teinte bleu) ou masculin (teinte jaune), dans lequel a &eacute;t&eacute; identifi&eacute; au moins une opinion-&eacute;motion-sentiment (OSE). Une barre blanche renvoie, soit &agrave; un tour de parole sans OSE identifi&eacute;e, soit &agrave; un tour de parole pour lequel la qualit&eacute; de la reco est inf&eacute;rieure au seuil pr&eacute;-d&eacute;fini (0,5).</p><p>L'intensit&eacute; de la teinte renvoie &agrave\; une polarit&eacute\; majoritairement positive (clair), n&eacute\;gative (sombre), ou parfaitement &eacute\;quilibr&eacute\;e entre positif et n&eacute\;gatif (moyen).";
print S " Si un concept de la classe malveillance a &eacute;t&eacute; identifi&eacute; dans un tour de parole, un point d'exclamation est affich&eacute; sur la barre verticale." if ($afficheMalveillance eq "o");
print S "</p>\n";
